...
ax2.scatter(table_signal.index[table_signal['Actual fault'] > 0] * 0.01, table_signal['Faulty signal'][table_signal['Actual fault'] > 0], 
            marker = 'x', color = 'green', s = 72, label = 'Actual fault')
ax2.scatter(table_signal.index[table_signal['Observed fault'] == 1] * 0.01, table_signal['Faulty signal'][table_signal['Observed fault'] == 1], 
...
for i in range(1, 6):
    ax3.scatter(table_signal.index[table_signal['Actual fault'] == i] * 0.01, table_signal['Faulty signal'][table_signal['Actual fault'] == i], 
                marker = 'x', color = colors_list_actual[i - 1], s = 72, label = f"Actual {fault_types[i - 1]}")
    ax3.scatter(table_signal.index[table_signal['Predicted fault'] == i] * 0.01, table_signal['Faulty signal'][table_signal['Predicted fault'] == i], 
